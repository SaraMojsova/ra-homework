...
const applyArray=[this.field, `${this.field??'isLoggedIn'} field doesn\'t exist on ${this?'user1':'user2'}`] // ${this?'user1':'user2'} not sure how to handle if it prints user1 or user2

print.apply(user)
getUserData.apply(user,applyArray)
console.log('---------------------------------')
print.apply(user2)
getUserData.apply(user2,applyArray)
...
printBind1();
printBind2()
getUserDataBind1(this.field, `${this.field??'isLoggedIn'} field doesn\'t exist on user1`)
getUserDataBind2(this.field, `${this.field??'isLoggedIn'} field doesn\'t exist on user2`)
